...
describe('normalized work queue overview', () => {
  beforeEach(() => {
    getAllWorkflowDeskItemsMock.mockResolvedValue([
      ...
      { ...base, contentType: 'article', id: 'breaking', title: 'Breaking article', status: 'copy_edit', priority: 'normal', isBreaking: true, assignedToId: 'copy-1', editHref: '/admin/articles/breaking/edit', updatedAt: '2026-07-14T09:00:00Z' },
    ]);
  });
  ...
  it('returns role-aware view counts and URL-filtered views for an admin', async () => {
    ...
    expect(overview.viewCounts.unassigned).toBe(1);
  ...
  it('does not expose other desks records to a reporter', async () => {
    ...
    expect(overview.items.map((item) => item.id)).toEqual(['mine']);
  });
});
